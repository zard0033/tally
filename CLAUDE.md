...
- 測試：`npx vitest run`＝邏輯層（動公式、捨入、日期必跑）；`npm run e2e`＝UI 回歸，共 49 條，分九個檔（`tally.spec.ts` 累積狀態的全份回歸、`interaction.spec.ts` 13 條可單獨跑的互動路徑、`vendor-autocomplete.spec.ts`、`meal-exit-animation.spec.ts`、`quota-warning.spec.ts`、`inline-edit.spec.ts`、`daily-goal.spec.ts`、`food-library.spec.ts`、`settings-nav.spec.ts`；改 UI 必跑全部，fetch 全 stub 不打真後端）。單跑一條：`npx playwright test interaction -g "關鍵字"`。**`workers` 釘 4，不要拿掉**——平行單位是檔案，worker 數會跟著 spec 檔數長，9 個檔同時打單一 Vite dev server 會讓它 OOM 死掉（症狀長得像瀏覽器崩潰，見 playwright.config.ts 註解）。**跑 e2e 前先確認 5500 沒有殘留的 dev server**：`reuseExistingServer: true` 會接手孤兒 server 卻不管它的生死。
