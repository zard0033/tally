...
- 測試：`npx vitest run`＝邏輯層（動公式、捨入、日期必跑）；`npm run e2e`＝UI 回歸，共 31 條，分六個檔（`tally.spec.ts` 累積狀態的全份回歸、`interaction.spec.ts` 13 條可單獨跑的互動路徑、`vendor-autocomplete.spec.ts`、`meal-exit-animation.spec.ts`、`quota-warning.spec.ts`、`inline-edit.spec.ts`；改 UI 必跑全部，fetch 全 stub 不打真後端）。單跑一條：`npx playwright test interaction -g "關鍵字"`。
...
- [ ] 改 UI → `npm run e2e` 綠（31 條，見上方六個檔）；webkit 393×745 截圖看過，token 無黑色 fallback。**本專案只有淺色一套**（DESIGN.md 明文不做暗色模式），所以沒有暗色那半邊要截
