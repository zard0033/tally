...
- 測試：`npx vitest run`＝邏輯層（動公式、捨入、日期必跑）；`npm run e2e`＝UI 回歸，共 49 條，分九個檔（`tally.spec.ts` 累積狀態的全份回歸、`interaction.spec.ts` 13 條可單獨跑的互動路徑、`vendor-autocomplete.spec.ts`、`meal-exit-animation.spec.ts`、`quota-warning.spec.ts`、`inline-edit.spec.ts`、`daily-goal.spec.ts`、`food-library.spec.ts`、`settings-nav.spec.ts`；改 UI 必跑全部，fetch 全 stub 不打真後端）。單跑一條：`npx playwright test interaction -g "關鍵字"`。**e2e 跑的是 build 後的靜態 preview（port 5501），不是 dev server**——所以它會自己先 build 一次，也不會跟你開著的 `npm run dev`（5500）搶 port，兩邊可以同時開。**`workers` 釘 2，不要拿掉**：那是這台機器在一般桌面負載下能同時開的 WebKit 數量，調高會偶發崩在 `Cannot find parent object page@…`（來龍去脈見 playwright.config.ts 註解）。
...
- [ ] 改 UI → `npm run e2e` 綠（49 條，見上方九個檔）；webkit 393×745 截圖看過，token 無黑色 fallback。**本專案只有淺色一套**（DESIGN.md 明文不做暗色模式），所以沒有暗色那半邊要截
