...
- **app icon 已結案（v2.26，全文在 DESIGN.md v2.19–v2.26）**：透明底兩種模式都被填黑，
  別再試。教訓：① 症狀只出現在單一 app 不代表原因在那個 app（對照組省掉四輪）
...
1. **iOS 鍵盤三題——v2.34 待真機驗**（v2.31–v2.33 的 `intake.name` 與框線同色兩項
   ...
   **v2.34 的修法對三題都下注在同一個開關**（`repositionInputs={false}`），理由見
   ...
   收鍵盤。**每一步都截圖，讀數列的 `sheet inline` 應該全程是 `-`**（有值＝ vaul 還在寫
   inline style，那這個 flag 沒吃到）。
   **兩個判別實驗，決定 ③ 的真兇是 vaul 還是 Autocomplete**（現成畫面，不必寫 code）：
   ⓐ **食品庫的就地編輯**（有 Autocomplete＋form、無 vaul）箭頭斷不斷——斷→指向
   Autocomplete；順→指向 vaul。ⓑ **設定頁的記體重 sheet**（無 vaul、無 Autocomplete，
   `--kb` 單獨執政）——它就是「修好之後該有的樣子」的活樣本，鍵盤行為若已經是對的，
   v2.34 的方向就先被實證了。
...
③ **懷疑「我寫的 CSS 沒生效」時，先查有沒有第三方在寫 inline style**（v2.34：讀
`node_modules/vaul` 的原始碼三分鐘就見底，前兩輪都在重讀自己的 code 與猜 Safari）。
