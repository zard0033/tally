...
**第 1、2 步已完成**（前端骨架 ＋ 今日頁）。實作前必讀 `DESIGN.md`（尤其「禁止事項」與「已否決的做法」兩節）。

1. ~~前端骨架：登入頁（Google OAuth）→ 今日頁~~ ✅
2. ~~今日頁實作~~ ✅ 常態／破表／空狀態／混合狀態（單一營養素破表）／中間餐次空著五屏都驗過
3. **← 下一步**：「記一筆」流程 — **樣張還沒設計**，要先走 ➊ 決策樣張再實作：選餐別 → 搜尋食物 → 填份量 → 找不到則新增食物
4. 刪除當天某筆
5. 設定頁的編輯功能（第一版的唯讀版已隨骨架完成）

**接上「記一筆」時要一起解開的**（程式裡都標了 `ponytail:`）：`app.js` 的 `$('btn-add').disabled`、
