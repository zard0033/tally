...
- **[新需求 2026-08-13 · 未開工] 拍照讀營養標示自動填表**（研究＋真實照片實測各一輪）：
  **模型選 `google/gemini-2.5-flash`，走使用者既有的 OpenRouter 帳號**（原本供 Hermes 用；
  已另開一把獨立 key，**但花費上限還沒設**——正式接後端前必補，否則迴圈或被打會吃掉整個帳號
  餘額、Hermes 一起遭殃）。**免費模型這條路已實測否決**：`gemma-4-31b:free` 撞 429、
  `nemotron-nano-12b-vl:free` 整組錯位（把脂肪讀進蛋白質欄），Flash 一次僅約台幣 0.03 元。
  **實測結果（單一樣本，維力炸醬麵）**：Flash 讀出的四個數字用 **4/9/4 反算＝559.1，與它讀出的
  熱量完全相同**——這給了一個免費的自我檢查，但**只能當寬容差警示不能當硬閘門**（DESIGN.md
  已記「標示熱量與 4/9/4 本來就對不上」）。兩個模型的 `serving_g` 都是 null＝提示詞要補。
  **架構（不因換 provider 而改變）**：key 不能進 public bundle → 必須一層 Supabase Edge Function
  ＋**驗 Supabase JWT**；拍照本身零成本（`<input type="file" accept="image/*"
  capture="environment">` iOS Safari 直接開相機），與 PWA 無相依。**已否決的兩條路**：
  Tesseract.js 純前端（讀密集表格公認不可靠）、掃條碼查 Open Food Facts（iOS Safari 不支援
  `BarcodeDetector`；OFF 全文搜尋無 CORS；台灣覆蓋率兩次查詢都撞主 API 502 沒驗成——真要重驗
  是「條碼精確查詢」端點而非全文搜尋）。台灣無官方路：data.gov.tw 8543 是食材成分庫非標示庫。
  **還沒問過的先決問題**：食品庫已能存常吃品項，拍照的價值只在「第一次吃的新品項」——頻率不高
  的話最懶的答案是不做。探針 `<scratchpad>/ocr-probe.py`（三模型並排比對，key 走環境變數）。
