...
- **測試**：`npx vitest run` 64/64；`npm run build`／`npm run lint` 乾淨；`npm run e2e`
  **74/74 全綠**（約 1.3 分，十個 spec 檔，各檔涵蓋範圍見 CLAUDE.md）。**條數以實跑輸出為準**
  ——2026-08-12 又漏算一條被 review 抓到，改文件前先跑一次比心算快；而且**條數（74／10 檔／64）
  有三份文件在寫**（CLAUDE.md、本檔、兩份 README），改條數要一次改到底；
...
- **[新需求 2026-08-13 · 未開工] 預先記錄明天的飲食**：情境是「今天先安排明天吃什麼，
  ...
  **屬中大型、走 dev-flow。**
- **[新需求 2026-08-13 · 未開工] 拍照讀營養標示自動填表**（已做完可行性研究，結論如下）：
  **沒有「完全免費且不需後端」的可靠路**。三條路各自的死角：① **Tesseract.js 純前端 OCR**
  ——`chi_tra` 語言包 1.58MB gzip、可自託管，但**公認讀密集表格不可靠**，產出的是要人工校對
  的數字，比手打還煩。② **掃條碼查 Open Food Facts**——`BarcodeDetector` 在 **iOS Safari
  完全不支援**（要打包 zxing 類函式庫）；OFF 的**全文搜尋 API 實測沒有 CORS header**，
  瀏覽器直接 fetch 會被擋；**台灣品項覆蓋率至今未驗成**（2026-08-13 agent 與主對話兩次
  查詢都撞上 `world.openfoodfacts.org` 主 API 502，`search.`／`static.` 子網域同時正常
  ＝該服務的可用性本身也是風險）。**要重驗的是「條碼精確查詢」端點，不是全文搜尋**——
  兩者可靠度完全不同，用 OCR 認出的商品名去全文搜尋，回來第一筆多半是雜訊（實測 3 查詢 0
  精確命中）。③ **多模態視覺模型**——辨識力才是「拍了就填好」要的，免費額度存在（Groq
  限制最明確且據文件預設不訓練；Gemini 免費層資料預設會被人工審閱用於訓練），**但 key
  不能進 public repo 的 bundle，一定要一層後端代理**。**所以「免費」省得掉錢、省不掉後端**
  ——Supabase 免費方案含 Edge Functions，代價是工不是錢，且**必須驗 Supabase JWT**，
  否則等於開放任何人白用你的額度。**台灣沒有官方路**：data.gov.tw 8543 是「食材」成分
  資料庫（白米、雞胸肉），不是包裝食品標示。拍照本身零成本（`<input type="file"
  accept="image/*" capture="environment">` 在 iOS Safari 直接開相機），與 PWA **無相依**。
  **還沒問過的先決問題**：食品庫已經能存常吃品項，拍照辨識真正的價值只在「第一次吃的新品項」
  ——那個頻率若不高，最懶的答案是不做。
...
demo 才是這類決策的正確載體**，而且要我先做、不是等他要求）。⑥ **v2.39 印證：停手時把「真機
若也沒效，從哪裡試起」寫下來，下一輪才有得對帳**（那條後來不必用上，因為真兇是環境）。
