...
最後更新：2026-08-13（**辨識失敗的根因找到並修完：前後端逾時的方向訂反了。前端待 push、
函式待部署，兩邊都上線才算收——見續點 2**）
...
- **功能**：第一版全數上線＋額度預警提示＋今日頁品項就地編輯＋每日目標計算引擎＋食品庫管理
  ...
  **已上線＝v2.41，v2.42 已完成待 push**（拍照讀營養標示：v2.40 功能本體、v2.41 圖片上限、
  v2.42 逾時方向訂反與 StrictMode 守衛＋辨識秒數 `[追加]`）。
...
- **測試**：`npx vitest run` 97/97；`npm run build`／`npm run lint` 乾淨；`npm run e2e`
  **78/78 全綠**（約 1.3 分，十一個 spec 檔，涵蓋範圍見 CLAUDE.md）。**條數以實跑輸出為準**——
  2026-08-12 又漏算一條被 review 抓到；且條數（78／11／97）有四份文件在寫（CLAUDE.md、本檔、
...
2. **拍照辨識：v2.42 待上線**（規格見 spec.md，取捨見 DESIGN.md v2.40–v2.42）。桌面實測已成功
   （13–22 秒、讀數正確）。**本輪未完的第一件事**：前端 push ＋ **函式部署**
   （`npx supabase login` 要使用者跑，指令 `npx supabase functions deploy read-label
   --project-ref bpnucfejoiazmsnsuzdb --use-api --no-verify-jwt`，**兩個旗標缺一不可**）。
   兩邊都上線才收——只上一邊會留下「50–60 秒無聲」或「使用者仍卡 15 秒」的殘留band。
   驗收＝`edge_logs` 出現 version 6，且下一次真機辨識產出第一行 `200 ok {ms, usage}`。
   **剩下的**：① 用那行 `usage` 的 `reasoning_tokens` 決定要不要關 thinking／縮圖（使用者要測，
   探針腳本還沒寫：同一張圖 800／1000／1200 × reasoning 開關，各三次取中位數，**要用他的
   OpenRouter key、不進對話**）② **iOS 鍵盤上下箭頭未驗（只能真機）**：`<form>` 與欄位間多了
   一層 `<fieldset>`，v2.33 的教訓正是「沒有 form 時 Safari 靠 DOM 相鄰性猜分組」。
   ③ `supabase/functions/**` 不在 `tsconfig.app.json` 的 include 內，**型別錯誤 CI 攔不到**。
   ④ sheet 開啟時 `#root` 上 `aria-hidden` 但焦點還留在 `.lib-fab`（瀏覽器主動警告，讀屏找不到
   那顆鈕）——**不是本輪造成的**，零碎項。
