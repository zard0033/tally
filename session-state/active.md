...
最後更新：2026-08-13（**辨識失敗已修完並上線（v2.42–v2.44，函式 version 7）；加速三條路實測
全否決。工作區留著 v2.45（鍵盤切焦點捲不過去）待 review——見續點 2**）
...
1. **PWA / service worker：★核可已通過，實作零進度**（2026-08-05 收工於此）。
   ...
   **實作第一步必須先實測「離線時 supabase-js 會不會把 App 踢回登入頁」**——唯一可能推翻整個設計
   的未知數（分歧點在 supabase-js 要不要為了 refresh 打網路；探針隨 scratchpad 消失，要重寫）。
...
2. **拍照辨識：v2.42–v2.44 均已上線**（函式 version 7）。加速三條路全數實測否決、思考維持開啟，
   ...
   ③ v2.45 只修了「切換焦點」那條路；**第一次點欄位叫出鍵盤那一下還沒驗**（`--kb` 要等
   `visualViewport` 事件才更新，可能算到縮之前的版面）。
...
4. **待刪（v2.38 一次性產物，等使用者點頭）**：[ios-gap-analysis.md](ios-gap-analysis.md)、
   `_design-sample/ios-tuning-compare.html`。**刪前先 grep 檔名**——三處引用（DESIGN.md「對照 demo
   現況」要改、v2.38 那條是歷史留著、`app.css` scrim 註解自行判斷）。
...
它們有沒有共同的上游**（三題最後同一個開關）。⑤ **交付形式錯了，內容再對也沒用**
