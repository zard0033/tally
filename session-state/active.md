...
最後更新：2026-08-12（v2.31 已 push；v2.32 兩件待 push，都要真機驗）
...
- **食物表單在 iOS 被鍵盤蓋住 → 修法已寫、待真機驗（v2.32）**：`.sheet` 的 `bottom` 改吃
  ...
  脂肪／碳水那排要看得見（或至少捲得到），「加入食品庫」浮在鍵盤正上方。**順帶驗上下箭頭**
  ——診斷認定它「跳得不順」只是因為跳到看不見的地方，若縮上來之後仍然不順，那才是另一個問題
  （嫌疑：店家 Autocomplete 的 `openOnInputClick`）。

## 下次續點

1. **下一次開手機要一口氣驗三件事**（v2.31 已 push 於 `73c9b14`；v2.32 兩件待 push）：
   ① **`intake.name` 在線上 DB 存不存在**——e2e 全打 stub，`select` 現在含 `name`，
   今日頁載不出來就是它；② **控件框線**（v2.32）統一成調深的 `--rule-field #9A8069`，
   四排該同色；③ **iOS 鍵盤**（v2.32）新增食物表單底部欄位不再被壓掉，驗法見待決那條。
   ①②③ 都要真機是刻意併成同一次部署的。
