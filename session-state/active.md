...
最後更新：2026-08-12（**iOS 鍵盤三題全數真機驗過、結案**；v2.37 移除任務完成的 debug 面板，待 push）
...
- **功能**：第一版全數上線＋額度預警提示＋今日頁品項就地編輯＋每日目標計算引擎（v2.21→
  ...
  **v2.34–v2.36 iOS 鍵盤三題**（走 vaul 的 sheet 一律 `repositionInputs={false}`
  ＋ sheet 的 top 吃 `--vvtop`）——**三題已全數真機驗過、結案**；診斷用的 `?debug`
  讀數列任務完成，v2.37 已整組移除（要用從 git 撿）。
...
1. **「做起來更有 iOS app 質感」——已談定範圍，尚未開工**（2026-08-12 使用者提出）。
