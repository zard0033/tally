...
最後更新：2026-08-13（**辨識失敗已修完並上線（v2.42／v2.43，函式 version 7）；加速三條路
實測全否決。工作區留著一個 sheet 焦點修正待 review——見續點 2**）
...
- **功能**：第一版全數上線＋額度預警提示＋今日頁品項就地編輯＋每日目標計算引擎＋食品庫管理
  ...
  **已上線＝v2.43**（拍照讀營養標示：v2.40 功能本體、v2.41 圖片上限、v2.42 逾時方向訂反與
  StrictMode 守衛＋辨識秒數 `[追加]`、v2.43 加速三條路實測否決）。**v2.44 在工作區待 review。**
...
- **測試**：`npx vitest run` 97/97；`npm run build`／`npm run lint` 乾淨；`npm run e2e`
  **79/79 全綠**（約 1.3 分，十一個 spec 檔，涵蓋範圍見 CLAUDE.md）。**條數以實跑輸出為準**——
  2026-08-12 又漏算一條被 review 抓到；且條數（79／11／97）有四份文件在寫（CLAUDE.md、本檔、
...
- **今天連翻四次案，每次的解藥都是「一個獨立來源的量測」**（v2.43 全文在 DESIGN.md）：token 數
  解釋耗時 → 被第二次量測翻；差異歸因負載 → 被按路徑分組翻；路徑差異 → 被 OpenRouter 的
  activity export 翻（**探針把 `ms` 結算寫在 `await fetch()` 正下方，量到的是 TTFT**）；
  「關掉思考會讀錯」→ 被真機照片翻。**共同形狀：拿一個能解釋數字的故事當因果，而沒先問
  「這些量測是不是在同一個條件下取的」、「量測工具本身驗過嗎」。**
...
- **拍照辨識**（需求／AC／模型選型／已否決的路徑全文在 [spec.md](../spec.md)，不複製第二份）
  進度移到續點 2。前輪的探針腳本隨 scratchpad 消失，要用得重寫。
...
2. **拍照辨識：v2.42／v2.43 均已上線**（前端 `f7e75cd`、函式 version 7）。加速三條路全數實測
   否決、思考維持開啟，全文與數據在 DESIGN.md v2.43。**剩下的**：① **iOS 鍵盤上下箭頭未驗
   （只能真機）**：`<form>` 與欄位間多了一層 `<fieldset>`，v2.33 的教訓正是「沒有 form 時
   Safari 靠 DOM 相鄰性猜分組」 ② `supabase/functions/**` 不在 `tsconfig.app.json` 的 include
   內，**型別錯誤 CI 攔不到** ③ 已知風險未處理：**拍太遠會得到看起來合理、實際差一倍的數字**
   （82 讀成 140），app 察覺不到，只能靠拍照前提示「靠近拍」。
