...
最後更新：2026-08-05（v2.24／v2.25 已 push；v2.26 待 push＝icon 退回不透明米白底＋綠 T）
...
- **功能**：第一版全數上線＋額度預警提示＋今日頁品項就地編輯＋每日目標計算引擎（v2.21→
  ...
  **v2.24／v2.25 已 push**（`9133d74`）；**v2.26 待 push**（icon 退回不透明米白底
  ＋`#2A9D7A` 綠 T——v2.25 的透明底被實機推翻，見「已驗證事實」）。
...
- **iOS 深色主畫面對 web clip 的處理是「換掉背景、保留前景的顏色」**（2026-08-05 兩輪實測定論）：
  ...
  **透明底不是解法**：實測 iOS 把透明區填黑，而且**淺色模式也填黑**——v2.25 試過，
  ...
  說法在實機上不成立，別再引用。）
  **正解＝不透明淺色底**：淺色模式看到米白底綠 T，深色模式由 iOS 換底、綠 T 留著（v2.26）。
...
1. **push v2.26 後請使用者在實機重加一次主畫面圖示**，確認兩件事：淺色模式是米白底綠 T、
   深色模式是黑底綠 T。**待驗的假設只剩一條**——iOS 是「換底留前景」（有一次觀察支持），
   而不是「整張圖一起壓暗」；若是後者，深色模式的綠 T 會跟著變暗。
   若又不對，**不要再開第六輪猜測**：這條路的可用配色已經窮舉完（深色變體做不到），
   剩下的只是在「哪一組妥協可接受」之間選，那是使用者的品味判斷不是技術問題。
