# Tally — session-state 歸檔（2026-07）

> 這是 2026-07-30 「快照手術」前的完整流水帳歸檔（570 行），**不再更新**。
> 現況、待決、續點一律看 [active.md](active.md)；本檔只在需要考古某一輪的施工細節時才讀。
...
### 真機第三輪（2026-07-30 使用者回報）——最新，先讀這節

- **左滑手感 ✅ 過**（motion drag 手刻的採用定案）。**額度預警提示使用者裁定「先等等」**，暫不開工。
- **① 滑開時刪除鈕「圓角過大＋上下被截斷」→ 已修**。根因量到（playwright 探針）：`is-armed` 的
  `scale(1.18)` 把 44px 圓鈕撐成 51.9px，而 `.item-row` 是 `overflow: clip`（同一天為修水平溢位
  加的），上下各切掉 **4.0px**——兩個各自正確的修法撞在一起。修法＝紅圓改畫在
  `.item-delete::before`（`inset: 2px` ＝ 40px），預告改 `scale(1.1)` ＝ 44px 剛好貼齊列高；
  44px 觸控區與「只動 transform」都沒動。順帶：常態的圓從「與列上下緣相切」變成留 2px 呼吸。
- **② 復原提示條 → 已改成右下角 44px 深色 pill**（`--ink` 底、`--paper` 字、undo 箭頭＋「復原」、
  整顆可點、貼 CTA 上方 8px，實測 84×44）。三點回報一次解：太大塊（原 353×62）、位置浮在
  時間軸中段、不夠明顯（`--raised` 對 `--paper` 實算 1.14:1）。畫面上不再寫「已刪除」，
  只留 `role=status` 的 sr-only 文字——列消失本身就是回報。`.undo-bar button` 的 DOM 契約沒變，
  e2e 三條斷言不用改。
- **③ 待決策：「記一筆」CTA ＋ 分頁列「有點擠有點高」**。實測底部兩段合計 **138px = 745px 的 18.5%**
  （CTA 區 68 ＝ 8+52+8；分頁列 70 ＝ 4+54+12＋safe-area），中間只隔 12px。方向選項與取捨
  已列給使用者，**尚未選**。動它要一併改 DESIGN.md 的「主 CTA」與「分頁列」兩條。
- **驗證**：vitest 42/42、e2e 14/14（PASS 14/14）、tsc/oxlint 退出碼 0。DESIGN.md 已更新至 **v2.2**。
- 仍未驗（下次拿手機順手看）：apple-touch-icon 加主畫面、tabular-nums 對齊。

### 技術棧轉向（2026-07-29 使用者拍板）
...
#### verifier 複驗結果與回修（2026-07-29）——**第一版有兩個硬 bug，複驗抓到**

主對話的煙霧驗證（vitest／tsc／oxlint／e2e 全綠）**完全沒抓到這兩條**，兩條都是 fresh verifier
自己寫 playwright probe 實測出來的。這是「驗證不自驗」最直接的一次回本。

- **① 縱向捲動帶左偏會靜默刪除一筆（資料損失）**：門檻讀的是指標原始位移 `info.offset.x`，
  而 `dragDirectionLock` 判定成縱向時**列根本不會動、offset.x 照樣累積**。實測重現＝
  先下拉 20-90px 再左移 300px 放手：`.item-slide` 的 transform 全程 `none`（畫面毫無變化）、
  品項卻少一筆。改讀 `x` motion value——它是位移的唯一真相，鎖在 Y 軸時就是 0。
- **② 復原後那一列永遠刪不掉**：`deletingIds` 只加不減，而復原會把該列放回來，於是它的
  刪除鈕永久 disabled。刪除已改樂觀移除、按下去當場離開清單，**這個旗標不再有存在理由**，
  整個移除；同一列連點的守衛改放 App.tsx 比對待刪 id（那裡才是資料的真相）。
- **③ undo 還原用整包快照在連續刪兩筆時會假復活**：刪 A → 5 秒內刪 B → A 送出失敗 →
  畫面還原成「A、B 都在」，但 B 仍在待刪隨後真的被刪。改成「把那一筆插回**目前**清單」。
- 其餘回修：復原條改絕對定位（走版面流時每次刪除都把時間軸擠短 62px、抖一下）；
  刪除後焦點掉回 body 時轉到「復原」鈕；exit／layout FLIP／復原條補 `prefers-reduced-motion`
  （原本只有 drag 吸附有，實測 reduce 模式退場 282ms vs 正常 330ms＝等於沒降）；
  滑到底門檻鎖在 `dragConstraints` 之內（寬版面上 45% 會超過 280px 邊界）。
- **e2e 補三條回歸鎖**（現在 13 條）：undo 後可再刪（鎖 bug ②）、縱向拖曳不可刪（鎖 bug ①）、
  **純橫向拖到底必須真的刪掉**——最後這條是前一條的對照組，沒有它的話萬一拖曳路徑
  根本沒被觸發，「縱向不誤刪」會在無事發生的情況下假通過。
- **有意識保留（已在程式碼標 ponytail）**：`pagehide` 送的是普通 fetch，關分頁時可能被中止、
  那筆沒刪成，下次開 app 會再出現。**這是刻意選的失敗方向**——反過來做（先真的 DELETE、
  復原時反向 INSERT）在復原失敗時會直接損失資料，而這邊最壞只是「刪了又回來」。
  要根治得繞過 supabase-js 自己組帶 keepalive 的請求（sendBeacon 設不了 auth header）。
- **已知未覆蓋**：刪掉某餐**最後一筆**時整個 `<ul>` 隨餐次轉「待記錄」一起卸載，退場動畫
  不會跑（DOM 20ms 內就移除）。e2e fixture 每餐只有一筆，所以退場動畫其實從沒被測到——
  要覆蓋得先擴 fixture 讓某一餐有兩筆以上。

#### 第二輪 verifier 複驗（2026-07-29）——八項過六項，再回修三條

verifier 自己把 fixture 改成同一餐三筆（讓退場動畫與 layout FLIP 真的跑得到）再寫 probe 實測。
過的六項：縱向不誤刪（對照組確認拖曳真的會刪，不是假通過）、復原後可再刪、連續刪兩筆的
undo 資料正確、版面不再跳動（`.timeline` 356→356）、reduced-motion 真的降了（266ms→76ms）、
`x` 的 `style`／`animate` 不打架且復原條沒蓋住 CTA。

- **FAIL：焦點轉移完全沒生效**——我把判斷寫在 `undoOpen` 轉 true 的當下，但刪除是樂觀移除＋
  `AnimatePresence` 退場，那一刻**被刪那列的刪除鈕還在 DOM 上、仍持有焦點**，
  `activeElement === body` 永遠不成立。verifier 逐時刻取樣：+80ms 還在按鈕上、+200ms 才掉到
  body，而 effect 早就跑完了。改成等退場動畫跑完（`EXIT_MS + 40`）再判斷，**並補一條 e2e
  斷言鎖住這個時機**，不靠下一輪人工確認。
- **e2e 的「滑到底」對照組本身很脆**：`page.mouse.move(x, y, { steps })` 在冷頁面上不足以讓
  motion 的 drag 起手——verifier 用同一手勢連跑四次全都沒觸發拖曳、收尾等於一次 click。
  它在 e2e 裡會過是靠前面步驟累積的狀態。已改成 `slowDrag`（逐點移動＋每點停 16ms）。
  **這條很值得記住**：「證明另一條測試不是假通過」的對照組，自己也會靜默退化成不觸發。
- **`loadDay` 沒結清待刪會造成反向不一致**：刪 A 失敗 → 按「重試」跑 `loadDay` → 同在窗內的 B
  被一起撈回畫面 → B 的計時器隨後照樣送出 DELETE → 畫面看得到 B、伺服器上已經沒有。
  已在 `loadDay` 開頭補結清（用 ref 取最新的 `commitDelete`，因為它定義在 `loadDay` 之後）。
...
#### e2e 拆成可單獨跑的路徑，並因此抓到「拖曳開合根本沒生效」（2026-07-29）

上一節那條「測不起來所以留白」的斷言，拆開之後三分鐘就查到根因——**而且根因是產品 bug，
不是測試問題**。

- **新結構**：`e2e/harness.ts`（`openApp()` 一行開到有資料的今日頁，＋ `check`／`slowDrag`／
  `rowState()` 等共用件）；`e2e/tally.spec.ts` 保留原本 14 條累積狀態的完整回歸（那個
  「一輪走完不中斷」是刻意紀律，沒動）；**新增 `e2e/interaction.spec.ts`**，每條各自
  `openApp()`、互不影響，單獨跑約 3 秒：`npx playwright test interaction -g "關鍵字"`。
- **拆開後量到的事實**：拖 100px **放手前** `transform` 是 −100（拖曳完全正常），
  **放手後**彈回 0 且沒開。搭配「拖 160px 會直接刪除」，順序就清楚了——
  **瀏覽器補的 click 比 motion 的 dragEnd 先到**：click 先把列切開、dragEnd 再切一次，
  兩次抵銷成關閉。所以我用「在 dragEnd 記時間戳」擋 click 根本擋不到。
- **這是線上正在發生的 bug**：`6735d8d`（已部署）上，**左滑拖開一列是沒有反應的**，
  只有滑到底刪除會動。真機驗證時如果覺得「滑不開」，是這個，不是手感問題。
- **修法（第三版）**：旗標在 `onDragStart` 就立起（不管 click 在 dragEnd 前後都擋得住），
  清除交給 dragEnd 之後的計時器（不依賴任何不保證發生的事件，所以也不會卡住）。
  三個版本的失敗都寫在 `Today.tsx` 的註解裡了。
- **三條斷言已補**（拖過門檻要開／拖完的下一次點擊不可被吃掉／小幅拖曳要回彈不誤刪），
  第一條在修之前實測是 `false`，是真的紅轉綠。
