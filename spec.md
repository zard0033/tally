...
- **壓縮強度：1200px／q0.75 已實測通過**（2026-08-13，`IMG_5610.jpg` 伯朗奶茶隨身包——反光蓋住整片表格、塑膠袋彎曲、手持傾斜、標示僅佔畫面約 15%）。1200／1600／2048／原尺寸四組**全部六欄全中**，含標示小字「每一份量 17 公克」。**參數不敏感，取最省的 1200px（132 KB）**。原本的擔心「壓完表格只剩 270px 見方、數字 12px 高會讀不到」**不成立**。四組全過代表**上限沒被找到**——真要再壓得更小仍屬未知，但沒有理由去省那幾十 KB。日後若某張照片讀錯，第一個要調的就是這個數字（一行改動）。
