...
2. 選圖後前端壓縮：**長邊 ≤1200px、JPEG q0.85**。**payload 實測約 560KB（base64 752,775 字元，真機 iOS Safari）**——原本 AC 寫「<400KB」是拿本機 Pillow 的輸出推的，而瀏覽器的 canvas 沒有 Pillow 的 optimize，同樣像素大三倍，上線第一天就撞到後端上限。**這一條的教訓是方法不是數字：不要拿 A 工具的量測去設 B 工具的門檻。****尺寸換算**由 `src/lib/image.ts` 的 `fitWithin()` 承擔並有六條單元測試（含 mutation 檢查）；**位元組上界的真正 gate 在後端**——Edge Function 的 `MAX_IMAGE_CHARS` 會擋掉過大的 payload 並回 400，那條有測試。前端不另外斷言位元組數：要驗得有意義就得在瀏覽器裡真的產一張 4000px 的圖再壓，成本遠高於它擋到的風險（真正的上界已經在後端）。（**兩個旋鈕的成本不對稱**：token 由像素數決定 ── Qwen3.7 每 32×32 px 一個 token，1200px≈780 tokens／2048px≈2300 ──，而 **JPEG quality 完全不進 token 公式，只影響上傳位元組**。所以尺寸要省、品質不必省；原訂 q0.75 是把「省」用錯地方，q0.85 對小字多留一點餘裕而 token 成本不變。）
