...
### 74 end-to-end tests, and a UI change runs all of them

74 browser tests across 10 files, plus 64 unit tests covering the formulas, dates, and rounding. Any UI change runs the entire browser suite, not the file that looks related. The whole thing finishes in under two minutes, and "the relevant tests passed" is the most common way a regression gets shipped.
...
```bash
npx vitest run     # 64 unit tests: formulas, dates, rounding
npm run e2e        # 74 browser tests, 10 files (WebKit, 393x745)
